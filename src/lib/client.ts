...
var messageTypes = [
  [
    ...
    'SV_WEAPON_PICKUP',
    'SV_EMOTICON',
    'SV_VOTE_CLEAR_OPTIONS',
    'SV_VOTE_OPTION_LIST_ADD',
    'SV_VOTE_OPTION_ADD',
    'SV_VOTE_OPTION_REMOVE',
    'SV_VOTE_SET',
    'SV_VOTE_STATUS',
    'CL_SAY',
    'CL_SET_TEAM',
    'CL_SET_SPECTATOR_MODE',
    'CL_START_INFO',
    'CL_CHANGE_INFO',
    'CL_KILL',
    'CL_EMOTICON',
...
export class Client extends EventEmitter {
  ...
  constructor(ip: string, port: number, nickname: string, options?: iOptions) {
    ...
    this.UUIDManager.RegisterName(
      'what-is@ddnet.tw',
      NETMSG.System.NETMSG_WHATIS,
    );
    this.UUIDManager.RegisterName('it-is@ddnet.tw', NETMSG.System.NETMSG_ITIS);
    this.UUIDManager.RegisterName(
      'i-dont-know@ddnet.tw',
      NETMSG.System.NETMSG_IDONTKNOW,
    );

    this.UUIDManager.RegisterName(
      'rcon-type@ddnet.tw',
      NETMSG.System.NETMSG_RCONTYPE,
    );
    this.UUIDManager.RegisterName(
      'map-details@ddnet.tw',
      NETMSG.System.NETMSG_MAP_DETAILS,
    );
    this.UUIDManager.RegisterName(
      'capabilities@ddnet.tw',
      NETMSG.System.NETMSG_CAPABILITIES,
    );
    this.UUIDManager.RegisterName(
      'clientver@ddnet.tw',
      NETMSG.System.NETMSG_CLIENTVER,
    );
    ...
    this.UUIDManager.RegisterName(
      ...
      NETMSG.System.NETMSG_CHECKSUM_REQUEST,
    );
    this.UUIDManager.RegisterName(
      'checksum-response@ddnet.tw',
      NETMSG.System.NETMSG_CHECKSUM_RESPONSE,
    );
    this.UUIDManager.RegisterName(
      'checksum-error@ddnet.tw',
      NETMSG.System.NETMSG_CHECKSUM_ERROR,
    );
    this.UUIDManager.RegisterName(
      'redirect@ddnet.org',
      NETMSG.System.NETMSG_REDIRECT,
    );

    this.UUIDManager.RegisterName(
      'i-am-npm-package@swarfey.gitlab.io',
      NETMSG.System.NETMSG_I_AM_NPM_PACKAGE,
    );
  }
  ...
  private ResendAfter(lastAck: number) {
    ...
    this.lastSentMessages.forEach((msg) => {
      if (msg.ack > lastAck) toResend.push(msg.msg);
    });
  ...
  private Unpack(packet: Buffer): _Packet {
    var unpacked: _Packet = {
      twprotocol: {
        flags: packet[0] >> 4,
        ack: ((packet[0] & 0xf) << 8) | packet[1],
        chunkAmount: packet[2],
        size: packet.byteLength - 3,
      },
      chunks: [],
    };

    if (packet.indexOf(Buffer.from([0xff, 0xff, 0xff, 0xff])) == 0)
      // !(unpacked.twprotocol.flags & 8) || unpacked.twprotocol.flags == 255) // flags == 255 is connectionless (used for sending usernames)
      return unpacked;
    ...
    if (unpacked.twprotocol.flags & 8 && !(unpacked.twprotocol.flags & 1)) {
      // compression flag
      packet = huff.decompress(packet);
      if (packet.length == 1 && packet[0] == -1) return unpacked;
    }
  ...
  SendControlMsg(msg: number, ExtraMsg: string = '') {
    ...
    return new Promise((resolve, reject) => {
      if (this.socket) {
        var latestBuf = Buffer.from([
          0x10 + (((16 << 4) & 0xf0) | ((this.ack >> 8) & 0xf)),
          this.ack & 0xff,
          0x00,
          msg,
        ]);
        latestBuf = Buffer.concat([
          latestBuf,
          Buffer.from(ExtraMsg),
          this.TKEN,
        ]); // move header (latestBuf), optional extraMsg & TKEN into 1 buffer
        this.socket.send(
          latestBuf,
          0,
          latestBuf.length,
  ...
  SendMsgEx(Msgs: MsgPacker[] | MsgPacker, flags = 0) {
    if (this.State == States.STATE_OFFLINE) return;
    if (!this.socket) return;
    let _Msgs: MsgPacker[];
    if (Msgs instanceof Array) _Msgs = Msgs;
    else _Msgs = [Msgs];
    ...
    this.lastSendTime = new Date().getTime();
    var header: Buffer[] = [];
    if (this.clientAck == 0) this.lastSentMessages = [];
    _Msgs.forEach((Msg: MsgPacker, index) => {
      ...
      if (Msg.flag & 1) {
        ...
        header[index][0] =
          (((Msg.flag | 2) & 3) << 6) | ((Msg.size >> 4) & 0x3f); // 2 is resend flag (ugly hack for queue)
        if ((Msg.flag & 2) == 0)
    ...
    // let flags = 0;
    if (this.requestResend) flags |= 4;

    var packetHeader = Buffer.from([
      ((flags << 4) & 0xf0) | ((this.ack >> 8) & 0xf),
      this.ack & 0xff,
      _Msgs.length,
    ]);
  ...
  QueueChunkEx(Msg: MsgPacker | MsgPacker[]) {
    if (Msg instanceof Array) {
      for (let chunk of Msg) this.QueueChunkEx(chunk);
      return;
    }
    if (this.queueChunkEx.length > 0) {
      let total_size = 0;
      for (let chunk of this.queueChunkEx) total_size += chunk.size;
      if (total_size + Msg.size + 3 > 1394 - 4) this.Flush();
    }
    this.queueChunkEx.push(Msg);
    if (Msg.flag & 4) this.Flush();
  }
  ...
  async connect() {
    ...
    let predTimer = setInterval(() => {
      if (this.State == States.STATE_ONLINE) {
        if (this.AckGameTick > 0) this.PredGameTick++;
      } else if (this.State == States.STATE_OFFLINE) clearInterval(predTimer);
    }, 1000 / 50); // 50 ticks per second

    this.SendControlMsg(1, 'TKEN');
    let connectInterval = setInterval(() => {
      if (this.State == States.STATE_CONNECTING) this.SendControlMsg(1, 'TKEN');
    ...
    if (!this.options?.lightweight) {
      let inputInterval = setInterval(() => {
        ...
        if (this.State != States.STATE_ONLINE) return;
    ...
    let resendTimeout = setInterval(() => {
      if (this.State != States.STATE_OFFLINE) {
        if (
          new Date().getTime() - this.lastSendTime > 900 &&
          this.sentChunkQueue.length > 0
        ) {
          this.SendMsgRaw([this.sentChunkQueue[0]]);
        }
      } else clearInterval(resendTimeout);
    }, 1000);
    ...
    let Timeout = setInterval(() => {
      ...
      if (new Date().getTime() - this.lastRecvTime > timeoutTime) {
        ...
        this.emit(
          'disconnect',
          'Timed Out. (no packets received for ' +
            (new Date().getTime() - this.lastRecvTime) +
            'ms)',
        );
      ...
      this.socket.on('message', (packet, rinfo) => {
        ...
        if (packet[0] == 0x10) {
          if (packet.toString().includes('TKEN') || packet[3] == 0x2) {
            ...
            client_version.AddBuffer(
              Buffer.from('8c00130484613e478787f672b3835bd4', 'hex'),
            );
            ...
            if (this.options?.ddnet_version !== undefined) {
              ...
              client_version.AddString(
                `DDNet ${this.options?.ddnet_version.release_version}; https://www.npmjs.com/package/teeworlds/v/${version}`,
              );
            } else {
              client_version.AddInt(16050);
              client_version.AddString(
                `DDNet 16.5.0; https://www.npmjs.com/package/teeworlds/v/${version}`,
              );
            }

            var i_am_npm_package = new MsgPacker(0, true, 1);
            i_am_npm_package.AddBuffer(
              this.UUIDManager.LookupType(
                NETMSG.System.NETMSG_I_AM_NPM_PACKAGE,
              )!.hash,
            );

            i_am_npm_package.AddString(
              `https://www.npmjs.com/package/teeworlds/v/${version}`,
            );
        ...
        unpacked.chunks.forEach((chunk) => {
          if (
            !(chunk.flags & 2 && chunk.flags & 1 ? chunk.seq! > this.ack : true)
          )
            return; // filter out already received chunks
          if (chunk.flags & 1 && chunk.flags !== 15) {
            // vital and not connless
            this.lastCheckedChunkAck = chunk.seq!;
            if (chunk.seq === (this.ack + 1) % (1 << 10)) {
              // https://github.com/nobody-mb/twchatonly/blob/master/chatonly.cpp#L237
              this.ack = chunk.seq!;
              ...
              if (Bottom < 0) {
                if (
                  chunk.seq! <= this.ack ||
                  chunk.seq! >= Bottom + (1 << 10)
                ) {
                } else this.requestResend = true;
              } else {
          ...
          if (chunk.sys) {
            // system messages
            if (chunk.msgid == NETMSG.System.NETMSG_PING) {
              // ping
              let packer = new MsgPacker(
                NETMSG.System.NETMSG_PING_REPLY,
                true,
                0,
              );

              this.SendMsgEx(packer); // send ping reply
            } else if (chunk.msgid == NETMSG.System.NETMSG_PING_REPLY) {
              // Ping reply
            ...
            if (
              chunk.msgid >= NETMSG.System.NETMSG_SNAP &&
              chunk.msgid <= NETMSG.System.NETMSG_SNAPSINGLE
            ) {
              this
                .receivedSnaps++; /* wait for 2 ss before seeing self as connected */
              if (this.receivedSnaps == 2) {
                if (this.State != States.STATE_ONLINE) this.emit('connected');
              ...
              if (GameTick >= this.currentSnapshotGameTick) {
                ...
                if (this.SnapshotParts == (1 << NumParts) - 1) {
                  ...
                  let snapUnpacked = this.SnapUnpacker.unpackSnapshot(
                    mergedSnaps,
                    DeltaTick,
                    GameTick,
                    Crc,
            ...
            if (
              chunk.msgid >= NETMSG.System.NETMSG_WHATIS &&
              chunk.msgid <= NETMSG.System.NETMSG_CHECKSUM_ERROR
            ) {
              if (chunk.msgid == NETMSG.System.NETMSG_WHATIS) {
                ...
                if (uuid !== undefined) {
                  // IT_IS msg
                  packer.AddBuffer(
                    this.UUIDManager.LookupType(NETMSG.System.NETMSG_ITIS)!
                      .hash,
                  );
                  ...
                  // dont_know msg
                  packer.AddBuffer(
                    this.UUIDManager.LookupType(NETMSG.System.NETMSG_IDONTKNOW)!
                      .hash,
                  );
              ...
              if (chunk.msgid == NETMSG.System.NETMSG_MAP_DETAILS) {
                // TODO: option for downloading maps
                let unpacker = new MsgUnpacker(chunk.raw);
                ...
                packer.AddBuffer(
                  this.UUIDManager.LookupType(NETMSG.System.NETMSG_PONGEX)!
                    .hash,
                );
            ...
            if (chunk.msgid == NETMSG.Game.SV_VOTECLEAROPTIONS) {
              ...
              if (index > -1) this.VoteList = this.VoteList.splice(index, 1);
            }
            ...
            if (chunk.msgid == NETMSG.Game.SV_EMOTICON) {
              ...
              if (unpacked.client_id != -1) {
                unpacked.author = {
                  ClientInfo: this.SnapshotUnpacker.getObjClientInfo(
                    unpacked.client_id,
                  ),
                  PlayerInfo: this.SnapshotUnpacker.getObjPlayerInfo(
                    unpacked.client_id,
                  ),
                };
              }
            ...
            if (chunk.msgid == NETMSG.Game.SV_READYTOENTER) {
              var Msg = new MsgPacker(
                NETMSG.System.NETMSG_ENTERGAME,
                true,
                1,
              ); /* entergame */
              this.SendMsgEx(Msg);
